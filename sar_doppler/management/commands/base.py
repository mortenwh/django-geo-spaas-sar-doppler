"""Shared base for SAR Doppler processing management commands."""
import logging
...
def process(ds, manager_method_name):
    """Worker: call Dataset.objects.<manager_method_name>(ds) with timeout and heartbeat."""
    status = False
    uri = "(unknown)"
    db_locked = True
    while db_locked:
    ...
    try:
        updated_ds, status = getattr(Dataset.objects, manager_method_name)(ds)
...
class BaseSARDopplerCommand(BaseCommand):
    """Base command for batched parallel processing of SAR Doppler datasets.

    Subclasses must set ``manager_method`` to the name of the Dataset manager
    method to call on each dataset (e.g. ``"process"`` or
    ``"add_wind_waves_current"``).
    """
    manager_method = None
    ...
    def handle(self, *args, **options):
            ...
            for batch_start in range(0, len(datasets_list), nworkers):
                ...
                try:
                    ...
                    for ds in batch:
                        uri = ds.dataseturi_set.get(uri__endswith=".gsar").uri
                        pending.append(
                            (pool.apply_async(process, (ds, self.manager_method)), uri))
                    ...
                    while pending:
                        ...
                        for ar, uri in pending:
                            try:
                                ...
                                logging.error(
                                    "Task raised exception: %s (%s)" % (str(e), uri))
                                res.append((None, uri))
                    ...
                    def _cleanup(p):
                        p.terminate()
                        p.join()
                    cleanup_t = threading.Thread(target=_cleanup, args=(pool,), daemon=True)
                    cleanup_t.start()
                    cleanup_t.join(timeout=60)
        ...
        # False (e.g. already processed), which is not a failure.
        ...
        def _shutdown():
            listener.stop()
            file_handler.close()
        shutdown_t = threading.Thread(target=_shutdown, daemon=True)
        shutdown_t.start()
        shutdown_t.join(timeout=60)
